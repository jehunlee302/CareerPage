...
if %errorlevel% == 0 (
  ...
  :: Brief variants (no project bullet details)
  node scripts/yaml-to-latex.js --brief --lang en
  pushd latex
  xelatex -interaction=nonstopmode resume.tex > nul 2>&1
  xelatex -interaction=nonstopmode resume.tex > nul 2>&1
  popd
  if exist latex\resume.pdf copy /Y latex\resume.pdf data\resume-en-brief.pdf > nul
  node scripts/yaml-to-latex.js --brief --lang ko
  pushd latex
  xelatex -interaction=nonstopmode resume.tex > nul 2>&1
  xelatex -interaction=nonstopmode resume.tex > nul 2>&1
  popd
  if exist latex\resume.pdf copy /Y latex\resume.pdf data\resume-ko-brief.pdf > nul
  ...
  echo   ✓ data\resume-en-brief.pdf + data\resume-ko-brief.pdf
) else (
